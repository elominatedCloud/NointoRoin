  ...
  return (
    <>
      ...
      <nav
        ...
        className="fixed inset-x-0 bottom-0 z-20 border-t border-slate-200 bg-white/95 px-3 pb-[calc(10px+env(safe-area-inset-bottom))] pt-2 shadow-[0_-12px_30px_rgba(15,23,42,0.08)] backdrop-blur md:hidden"
      >
            ...
            return (
              <Link
                ...
                className={`flex min-h-[64px] min-w-[72px] flex-col items-center justify-center gap-1 rounded-2xl text-xs font-black ${
                  isActive ? "bg-emerald-800 text-white" : "text-slate-600"
                }`}
                href={link.href}
                key={link.href}
              >
