...
async function EmployerHome() {
  ...
  return (
      ...
      >
        <Link
            href="/"
            className="fixed right-6 top-6 z-50 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-black text-slate-700 shadow-sm transition hover:border-emerald-700"
        >
          로그아웃
        </Link>
        ...
        <section className="grid gap-4 md:grid-cols-4">
          <Metric
              label="등록 공고"
              value={`${stats.employerJobCount}건`}
              note="모집 중"
          />

          <Metric
              label="검토할 후보"
              value={`${stats.matchCount}명`}
              note="적합도 순"
          />

          <Metric
              label="시니어 프로필"
              value={`${stats.seniorProfileCount}명`}
              note="동의 완료"
          />
          ...
          {todayTasks.map((task, index) => {
            ...
            return (
                ...
                <span className="flex size-11 items-center justify-center rounded-xl bg-emerald-50 text-emerald-800">
                  ...
                  <p className="mt-3 min-h-12 font-bold leading-relaxed text-slate-700">
                    {task.description}
                  </p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-2 font-bold text-slate-600">
                  공고와 맞는 이유와 연락 가능 여부를 같이 봅니다.
                </p>
              </div>

              <Link
                  className="shrink-0 text-sm font-black text-emerald-800"
                  href="/employer/applications"
              >
                전체 보기
              </Link>
            ...
            <div className="mt-5 overflow-hidden rounded-xl border border-slate-200">
              {topMatches.map((match) => (
                  <div
                      className="grid gap-4 border-b border-slate-200 p-4 last:border-b-0 md:grid-cols-[1fr_90px_120px]"
                      key={match.id}
                  >
                    <div>
...
function Metric({
  ...
  return (
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {note ? (
            <p className="mt-2 text-sm font-bold text-emerald-800">{note}</p>
        ) : null}
      </div>
  );
}